...
class SMCIR():
    def __init__(self, args):
        ...
        self.criterion = nn.CrossEntropyLoss()  if self.args.train_mode == 'classification' else  nn.L1Loss()     # classification:nn.CrossEntropyLoss()          regresssion:nn.L1Loss()
        self.best_fid_v = float('inf')
        self.best_fid_a = float('inf')
    def do_train(self, model, dataloader,return_epoch_results=False):
        param_optimizer = list(model.named_parameters())
        no_decay = ["bias", "LayerNorm.weight"]
        MultiModalAttentionFusion_params = ['encoder.SMCIR_master']
        ...
        optimizer_grouped_parameters = [
            ...
            {"params": model.Model.bert.encoder.SMCIR.parameters(), 'lr': self.args.learning_rate, "weight_decay": self.args.weight_decay},
            {
                "params": [
                    p for n, p in param_optimizer if
            ...
            with tqdm(dataloader['train']) as td:
                for index,batch_data in enumerate(td):
                    ...
                    if self.args.train_mode == 'classification':
                        labels = labels.to(self.args.device).view(-1).long()
                    else:
                    ...
                    if self.args.train_mode == 'classification':
                        loss = self.criterion(logits.view(-1, self.args.num_labels), labels)  # Classification
                    else:

                        loss = self.criterion(logits, labels)+0.2*outputs[1]# regression
            ...
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    if self.args.train_mode == 'classification':
                        ...
                        loss = self.criterion(logits, labels)  #regression
