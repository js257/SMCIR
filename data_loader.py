...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        if self.args['custom_feature']:
            # use custom feature file extracted with SMCIR_master-FET
        ...
        self.labels = {
            ...
            # 'M': np.array(data[self.mode]['regression_labels']).astype(np.float32) #只有回归标签
        }
    ...
    def __truncate(self):
        ...
        def do_truncate(modal_features, length):
            if length >= modal_features.shape[1]:
                return modal_features  # 如果长度不小于当前长度，直接返回原始数据

            padding = np.zeros((modal_features.shape[2],))
            truncated_feature = []

            for instance in modal_features:
                # 先找出所有有效的特征
                valid_indices = [i for i, feature in enumerate(instance) if not (feature == padding).all()]

                if valid_indices:
                    # 找到第一个有效特征的索引
                    first_valid_index = valid_indices[0]
                    # 截取特征，确保不会越界
                    end_index = min(first_valid_index + length, modal_features.shape[1])
                    truncated_feature.append(instance[first_valid_index:end_index])
                else:
                    # 如果所有特征都是填充，则添加最后的长度
                    truncated_feature.append(instance[-length:])

            return np.array(truncated_feature)
